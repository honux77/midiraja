...
import static com.midiraja.ui.UIUtils.truncateAnsi;
...
public class LineUI implements PlaybackUI {
  ...
  public void runRenderLoop(PlaybackEngine engine) {
    ...
    term.println(String.format(
        "\033[7m Midiraja v%s - Terminal Lover's MIDI Player \033[0m",
    ...
    String indexStr =
        listSize > 1 ? String.format(" [%d/%d]", idx + 1, listSize) : "";
    ...
    try {
      while (engine.isPlaying()) {
        ...
        String timeStr = formatTime(currentMicros, incHrs) + "/" +
                         formatTime(totalMicros, incHrs);
        if (engine.isPaused()) {
        ...
        buffer.append(String.format(
            " %s | Spd: %.1fx(BPM: %5.1f) | Tr: %+d | Vol: %3d%% ", timeStr,
            engine.getCurrentSpeed(), effectiveBpm,
      ...
      if (term.isInteractive()) {
        ...
        term.print(
            Theme.TERM_CLEAR_TO_END); // Clear from cursor to end of screen
        term.print(Theme.TERM_SHOW_CURSOR +
                   "\033[?7h"); // Show cursor, re-enable auto-wrap
      } else {
        term.println("");
      }
