...
public class CompositePanel implements Panel {
    ...
    public CompositePanel(Panel... panels) {
        ...
        // It's specific for Now Playing, so let's just make it NowPlayingPanel or hardcode the distribution here.
    }

    public void setHeights(int... heights) {
        System.arraycopy(heights, 0, this.fixedHeights, 0, Math.min(heights.length, this.fixedHeights.length));
    }
    ...
    public void onLayoutUpdated(LayoutConstraints bounds) {
        ...
        for (int i = 0; i < panels.size(); i++) {
            int h = (i == panels.size() - 1) ? Math.max(0, bounds.height() - currentY) : fixedHeights[i];
            panels.get(i).onLayoutUpdated(new LayoutConstraints(bounds.width(), h, bounds.showHeaders(), bounds.isHorizontal()));
            currentY += h;
        }
    }

    @Override public void onPlaybackStateChanged() { panels.forEach(Panel::onPlaybackStateChanged); }
    @Override public void onTick(long currentMicroseconds) { panels.forEach(p -> p.onTick(currentMicroseconds)); }
    @Override public void onTempoChanged(float bpm) { panels.forEach(p -> p.onTempoChanged(bpm)); }
