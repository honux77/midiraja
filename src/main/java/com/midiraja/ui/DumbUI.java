...
/**
 * A non-interactive UI implementation. No input polling and minimal, static
 * output. Used for headless CI, script piping, or simple batch playback.
 */
  ...
  public void runRenderLoop(PlaybackEngine engine) {
    ...
    if (!headerPrinted) {
      ...
      if (listSize == 1) {
        ...
        term.println(
            String.format("Playing: %s to port '%s'", displayTitle, portName));
      } else {
        term.println(
            String.format("Playing %d files to port '%s'", listSize, portName));
      }
    ...
    if (listSize > 1) {
      term.println(String.format("  [%d/%d] %s - %s", (idx + 1), listSize,
                                 displayTitle, lengthStr));
    } else {
