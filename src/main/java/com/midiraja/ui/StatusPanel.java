...
public class StatusPanel implements Panel {
  private LayoutConstraints constraints =
      new LayoutConstraints(80, 1, false, false);
  ...
  public void render(ScreenBuffer buffer) {
    ...
    if (constraints.height() == 1) {
      String s =
          String.format("    %s%s / %s %s %3d%%  Vol:%d%% Spd:%.1fx Tr:%+d",
                        pauseIndicator, curStr, totStr, bar, percent,
                        (int)(volumeScale * 100), speed, transpose);
      buffer.append(truncate(s, constraints.width())).append("\n");
    } else if (constraints.height() >= 2 && constraints.height() <= 4) {
      String line1 =
          String.format("    Time: %s%s / %s  %s  %3d%%", pauseIndicator,
                        curStr, totStr, bar, percent);
      String line2 =
          String.format("    Tempo: %3.0f BPM (%.1fx) | Vol: %d%% | Trans: %+d",
      ...
      buffer.append(String.format("    Time:      %s%s / %s  %s  %3d%%\n",
                                  pauseIndicator, curStr, totStr, bar,
                                  percent));
      buffer.append(String.format("    Transpose: %+d\n", transpose));
      buffer.append(
          String.format("    Volume:    %d%%\n", (int)(volumeScale * 100)));
      String portInfo =
          context != null
              ? String.format("[%d] %s", context.targetPort().index(),
