...
public class CoreMidiProvider implements MidiOutProvider
{
    ...
    private static final MethodHandle CFRelease = LINKER.downcallHandle(
        CF_LOOKUP.find("CFRelease").orElseThrow(), FunctionDescriptor.ofVoid(ValueLayout.ADDRESS));
    ...
    private static final MethodHandle MIDIGetDestination =
        LINKER.downcallHandle(MIDI_LOOKUP.find("MIDIGetDestination").orElseThrow(),
            FunctionDescriptor.of(ValueLayout.ADDRESS,
                ValueLayout.JAVA_LONG) // ItemCount passed as long to be safe (uint32/unsigned long)
        );
    ...
    private static final MethodHandle MIDIOutputPortCreate =
        LINKER.downcallHandle(MIDI_LOOKUP.find("MIDIOutputPortCreate").orElseThrow(),
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS, ValueLayout.ADDRESS,
                ValueLayout.ADDRESS));
    private static final MethodHandle MIDISend =
        LINKER.downcallHandle(MIDI_LOOKUP.find("MIDISend").orElseThrow(),
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS, ValueLayout.ADDRESS,
                ValueLayout.ADDRESS));
    ...
    @SuppressWarnings("UnusedVariable") @Nullable private MemorySegment client = null;
    ...
    @Override public List<MidiPort> getOutputPorts()
    {
        ...
        try (Arena arena = Arena.ofConfined())
        {
            ...
            for (int i = 0; i < destCount; i++)
            {
                ...
                int status =
                    (int) MIDIObjectGetStringProperty.invokeExact(dest, kMIDIPropertyName, strRef);
                if (status == 0)
                {
